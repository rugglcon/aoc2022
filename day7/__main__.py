...
def part_one(filename):
    with open(filename) as f:
        ...
        dirs = process_dirs(lines)

    return sum([d.get_size() for d in dirs.values() if d.get_size() <= 100000])
    ...
    with open(filename) as f:
        ...
        return sorted(
            [
                ...
                if s.get_size() >= (30000000 - (70000000 - dirs["/"].get_size()))
            ]
        )[0]
...
if __name__ == "__main__":
    ...
    assert part_two(f"{cur_dir}/test.txt") == 24933642
